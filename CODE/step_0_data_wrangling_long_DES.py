...
OASIS2_lh = pd.read_csv(os.path.join(OASIS2_dir, 'lh_aparc_stats_long_a2009s.txt'), sep = "\t")
OASIS2_rh = pd.read_csv(os.path.join(OASIS2_dir, 'rh_aparc_stats_long_a2009s.txt'), sep = "\t")
...
OASIS2_lh = OASIS2_lh.rename(columns={"lh.aparc.a2009s.thickness":"ID_long"})
OASIS2_rh = OASIS2_rh.rename(columns={"rh.aparc.a2009s.thickness":"ID_long"})
...
OASIS2 = OASIS2_lh.merge(OASIS2_rh, on="ID_long")
#%%
ID_frame = OASIS2["ID_long"].str.split(pat=".", expand=True)
...
OASIS2= OASIS2.merge(OASIS2_demo, on="ID")
...
#OASIS2.columns = OASIS2.columns.str.replace("L_", "lh_", regex=True)
#OASIS2.columns = OASIS2.columns.str.replace("R_", "rh_", regex=True)
...
#%%
if write: 
    OASIS2.to_pickle(os.path.join(data_out_dir,"OASIS2_long_DES.pkl"))
    OASIS2_demented.to_pickle(os.path.join(data_out_dir,"OASIS2_demented_long_DES.pkl"))
    OASIS2.to_csv(os.path.join(data_out_dir,"csv/OASIS2_long_DES.scv"))
    OASIS2_demented.to_csv(os.path.join(data_out_dir,"csv/OASIS2_demented_long_DES.csv"))
#%% Update OASIS3
OASIS3_dir = '/project_cephfs/3022017.06/OASIS3/freesurfer'
OASIS3_lh = pd.read_csv(os.path.join(OASIS3_dir, 'lh_aparc_stats_long_2009s.txt'), sep = "\t")
OASIS3_rh = pd.read_csv(os.path.join(OASIS3_dir, 'rh_aparc_stats_long_2009s.txt'), sep = "\t")
...
OASIS3_lh = OASIS3_lh.rename(columns={"lh.aparc.a2009s.thickness":"long_ID"}) #why is this one different
OASIS3_rh = OASIS3_rh.rename(columns={"rh.aparc.a2009s.thickness":"long_ID"})
...
#%%
df_with_zeros = OASIS3[OASIS3.drop(columns=["diagnosis"]).eq(0).any(axis=1)]

OASIS3["diagnosis"] = OASIS3.diagnosis.astype(int)
OASIS3["diagnosis"] = OASIS3.diagnosis.astype(str)
...
if write:
    OASIS3_demented.to_csv(os.path.join(data_out_dir, "OASIS3_demented_long_DES.csv"),index=False )
    OASIS3.to_csv(os.path.join(data_out_dir, "OASIS3_long_DES.csv"),index=False )
    OASIS3_demented.to_pickle(os.path.join(data_out_dir, "OASIS3_demented_long_DES.pkl"))
    OASIS3.to_pickle(os.path.join(data_out_dir, "OASIS3_long_DES.pkl"))

#%% Add IMAGEN data set

# get Imagen data set
Imagen_lh = pd.read_csv(os.path.join('/project_cephfs/3022017.06/IMAGEN/freesurfer/lh_aparc_stats_all_a2009s.txt'),  sep='\t')
Imagen_rh = pd.read_csv(os.path.join('/project_cephfs/3022017.06/IMAGEN/freesurfer/rh_aparc_stats_all_a2009s.txt'),  sep='\t')
...
Imagen = Imagen[Imagen_lh["ID"].str.contains(r"\.long\.", na=False)]

Imagen = Imagen.loc[:, (Imagen != 0).any()]
...
Imagen['ID_visit'] = Imagen['ID_visit'].str.replace('BL','1')
Imagen['ID_visit'] = Imagen['ID_visit'].str.replace('FU2','2')
Imagen['ID_visit'] = Imagen['ID_visit'].str.replace('FU3','3')
...
#write
if write:
    Imagen.to_csv(os.path.join(data_out_dir, "csv/IMAGEN_long_DES.csv"),index=False )
    Imagen.to_pickle(os.path.join(data_out_dir,"IMAGEN_long_DES.pkl"))
#%% ABCD
# Male = 1, Female =2
...
ABCD_lh_stats = pd.read_csv('/project_cephfs/3022017.06/ABCD/freesurfer/lh_aparc_stats_long_a2009s.txt', sep ="\t")
ABCD_rh_stats = pd.read_csv('/project_cephfs/3022017.02/freesurfer/rh_aparc_stats_long_a2009s.txt', sep ="\t")
...
#ABCD_lh_stats.drop(['lh_MeanThickness'], axis=1, inplace=True)
...
#ABCD_rh_stats.drop(['R_MeanThickness'], axis=1, inplace=True)
...
ABCD_stats["Mean_Thickness"] = ABCD_stats[idp_cols].mean(axis=1)
...
if write:
    ABCD_full.to_pickle(os.path.join(data_out_dir,"ABCD_long_DES.pkl"))
    ABCD_full.to_csv(os.path.join(data_out_dir,"ABCD_long_DES.csv"))
#%% UKB
...
UKB_lh.columns = UKB_lh.columns.str.replace("&", "_and_", regex=True)
UKB_lh.columns = UKB_lh.columns.str.replace("_thickness", "", regex=True)

UKB_rh.columns = UKB_rh.columns.str.replace("&", "_and_", regex=True)
UKB_rh.columns = UKB_rh.columns.str.replace("_thickness", "", regex=True)

column_to_delete_from = "rh_S_temporal_transverse"  
...
UKB_rh = UKB_rh.iloc[:, :col_index+1] 
UKB_lh = UKB_lh.iloc[:, :col_index+1] 

UKB_lh_rh = UKB_lh.merge(UKB_rh, on = "ID")

filter = UKB_lh_rh['ID'].str.contains(".long.")
UKB_lh_rh = UKB_lh_rh[filter]
...
UKB_lh_rh["Median_Thickness"] = UKB_lh_rh[idp_cols].median(axis=1)
# remove template rows

# create a subject ID
UKB_lh_rh["ID_visit"] = UKB_lh_rh["ID"].str.split(pat =".", expand=True)[0]
UKB_lh_rh["ID_subject"] = UKB_lh_rh["ID_visit"].str.replace('_scan2', '')
UKB_lh_rh["ID_subject"]= UKB_lh_rh["ID_subject"].astype(str)
#%% convert race
...
#%% 
UKB = UKB_lh_rh.merge(UKB_race, on ="ID_subject",  how= "left")
...
if write:
    UKB.to_pickle(os.path.join(data_out_dir,"UKB_long_DES.pkl"))
...
HCP_Babies_rh = pd.read_csv("/project_cephfs/3022017.06/HCP_Baby/freesurfer/rh_aparc_stats_long_a2009s.txt", sep = "\t")
HCP_Babies_lh = pd.read_csv("/project_cephfs/3022017.06/HCP_Baby/freesurfer/lh_aparc_stats_long_a2009s.txt", sep = "\t")
...
filter = HCP_Babies['ID'].str.contains(".long.")
HCP_Babies = HCP_Babies[filter]
...
HCP_Babies["ID_subject"] = HCP_Babies.ID.str.split(pat=".", expand=True)[0]

#%%
HCP_Babies = HCP_Babies.merge(HCP_babies_pass,left_on = "ID_subject", right_on = "ID", how="left")
HCP_Babies = HCP_Babies[HCP_Babies["pass"]==1]

HCP_Babies.drop(columns=["ID_y"], inplace = True)
HCP_Babies = HCP_Babies.rename(columns={"ID_x":"ID"})
...
HCP_Babies["ID_scan"] = HCP_Babies["src_subject_id"] + "_"+ HCP_Babies["scan_date"]


#%%
HCP_Babies_age = HCP_Babies_age[["ID_visit", "age"]]
HCP_Babies_age = HCP_Babies_age.rename(columns={"ID_visit":"ID_scan"})
HCP_Babies_age.drop_duplicates(inplace=True)
HCP_Babies = HCP_Babies.merge(HCP_Babies_age,  on="ID_scan", how ="left")
...
HCP_Babies_sex.src_subject_id = HCP_Babies_sex.src_subject_id.astype(int)
HCP_Babies.src_subject_id = HCP_Babies.src_subject_id.astype(int)
...
HCP_Babies = HCP_Babies.rename(columns={"ID_subject":"ID_subjects2"})
HCP_Babies = HCP_Babies.rename(columns={"src_subject_id":"ID_subject"})

if write:
    HCP_Babies.to_pickle(os.path.join(data_out_dir, "HCP_Babies_long_DES.pkl"))
...
NIH_Babies.ID_visit = NIH_Babies.ID_visit.astype(int)

if write:
    NIH_Babies.to_pickle(os.path.join(data_out_dir, "NIH_Babies_long_DES.pkl"))
