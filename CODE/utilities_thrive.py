...
def process_data(idp_nr, measure, model_type = "SHASHb_1", atlas = "SC", full_dataset=False):
#model_type ="SHASHb_1"
#atlas = 'SC'
#idp_nr = 0
    ...
    if full_dataset:
        test_raw = pd.read_pickle(
           f'/project_cephfs/3022017.06/projects/lifespan_hbr/johbay/Velocity/DATA_pre_processed/train_retrain_full_models_{atlas}_adults_ADNI.pkl')
       
        batch_dir = f'/project_cephfs/3022017.06/projects/lifespan_hbr/johbay/Velocity/{model_type}_estimate_scaled_fixed_{atlas}_retrain_ADNI/batch_{idp_nr +1}/'
    
    else:
        test_raw = pd.read_pickle(
            f'/project_cephfs/3022017.06/projects/lifespan_hbr/johbay/Velocity/DATA_pre_processed/test_{atlas}_demented_adults_ADNI.pkl')
    
        batch_dir = f'/project_cephfs/3022017.06/projects/lifespan_hbr/johbay/Velocity/{model_type}_estimate_scaled_fixed_{atlas}_demented_adults_ADNI/batch_{idp_nr +1}/'
    ...
    return clean_df, data, data_all, data_demented, OAS2_0157
...
# Save all at once
    with open(os.path.join(batch_dir, "velocity_objects_new_Oct25.pkl"), "wb") as f:
    ...
    velocity_list_age, velocity = generate_thrive_lines_conservative(
        pred_r, start_year=0, end_year=98, velocity_length=2, start_z=-3, end_z=4, space_between_anchors=1, z_thrive=-1.96
    )
    ...
    for (this_x, this_y) in zip(batched(velocity_list_age, b_size), batched(q_for_thrivelines, b_size)):
          plt.plot(this_x, this_y, linewidth=0.5, color='red')
    ...
    plt.savefig(f"{velocity_dir}/velocity_plot_{measure}_{model_type}_ADNI_Oct25_down.png",bbox_inches='tight',dpi=300)
    plt.show()
    # del model 
    # del model_be
    # del model_be_long
...
def make_sparse_correlation_matrix(
    ...
    for k in range(1, max_band+1):
        ...
        for i in range(num_ages - abs(k)):
            ...
            vec1 = data[data["age"] == age_i]
            vec2 = data[data["age"] == age_j]
    ...
    for i in range(start_year, end_year, space_between_anchors):
        ...
        for j in range(start_z, end_z):
            if z_thrive is not None:
                ...
                step_results_thrive = list(get_thrive_lines_cpnservative(pred_r_snippet, current_result=j))
