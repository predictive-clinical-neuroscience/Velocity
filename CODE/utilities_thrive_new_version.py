...
# -*- coding: utf-8 -*-
"""
Created on Fri Feb 21 14:13:28 2025
...
def process_data(atlas = "SC", full_dataset=False):

    
    #full_dataset = False
    if full_dataset:
        test_raw = pd.read_pickle(
           f'/project_cephfs/3022017.06/projects/lifespan_hbr/johbay/Velocity/DATA_new_version/{atlas}/train_retrain_full_models_{atlas}_adults_ADNI.pkl')
       
        z_scores = f'/project_cephfs/3022017.06/projects/lifespan_hbr/johbay/Velocity/CODE_new/PCNtoolkit/examples/resources/hbr_SHASH/save_dir_{atlas}_all_regions_full_datset/results/Z_test.csv'
    
    else:
        test_raw = pd.read_pickle(
            f'/project_cephfs/3022017.06/projects/lifespan_hbr/johbay/Velocity/DATA_new_version/{atlas}/test_{atlas}_demented_adults_ADNI.pkl')
    
        z_scores = f'/project_cephfs/3022017.06/projects/lifespan_hbr/johbay/Velocity/CODE_new/PCNtoolkit/examples/resources/hbr_SHASH/save_dir_{atlas}_all_regions/results/Z_test.csv'
    ...
    Z_measures_original = pd.read_csv(z_scores)
    Z_measures = Z_measures_original.sort_values(by="observations").reset_index(drop=True)
    ...
    # prune ()
    subject_counts = data['ID_subject'].value_counts()

    # Step 2: Find subjects that appear exactly 2 times
    subjects_exactly_2 = subject_counts[subject_counts == 2].index.tolist()
    ...
    # Step 3: Find subjects from those that are in ABCD or IMAGEN
    subjects_to_delete = data[
    (data['ID_subject'].isin(subjects_exactly_2)) & 
    (data['site'].isin(['ABCD', 'IMAGEN']))
    ]['ID_subject'].unique()
    
    data["age"] = data["age"].round()
    data["age"] = data["age"].astype(int)

    # Step 4: Remove them from the original DataFrame
    clean_df = data[~data['ID_subject'].isin(subjects_to_delete)]
    print(clean_df["site"].value_counts())
    
    clean_df["age"] = clean_df["age"].round()
    clean_df["age"] =  clean_df["age"].astype(int)
        
    clean_df["ID_visit"].astype(int)
    clean_df["ID_subject"].astype(str)
    ...
    #     plt.plot(group_data['age'], group_data[measure])
    # plt.show()
    
    # test_raw["which_dataset"] = pd.to_numeric(test_raw["which_dataset"], errors="coerce")
    
    # test_raw_demented = test_raw.loc[test_raw["which_dataset"] == 4,[measure, "ID_subject","age"]]
    ...
    return clean_df, data, data_all, data_demented 
    #, OAS2_0157


def make_velocity_plots(idp_nr, measure, data, selected_sex = 'male', model_type='SHAHSHb_1', atlas = "SC", re_estimate = True, full_dataset= False):
    
    #full_dataset = False
    if full_dataset:        
        out_dir = f'project_cephfs/3022017.06/projects/lifespan_hbr/johbay/Velocity/CODE_new/PCNtoolkit/examples/resources/hbr_SHASH/save_dir_{atlas}_all_regions_full_datset/results/'
    else: 
        out_dir = f'/project_cephfs/3022017.06/projects/lifespan_hbr/johbay/Velocity/CODE_new/PCNtoolkit/examples/resources/hbr_SHASH/save_dir_{atlas}_all_regions/results/'
    ...
    velocity_dir = os.path.join(out_dir,'Velocity', measure)
    if not os.path.exists(velocity_dir):
        os.makedirs(velocity_dir, exist_ok=True)
    
    max_offset =10
    #A, Phi = make_correlation_matrix(data=data, measure = measure, re_estimate=True)
    A_sparse, Phi_sparse = make_sparse_correlation_matrix(data=data, measure = measure, re_estimate=True, max_band=max_offset)
    
   
    # Create regression matrices for different gaps
    reg_matrices = [create_regression_matrix(gap, Phi_sparse) for gap in range(1, max_offset)]
    ...
    #X_pred = create_regression_matrix(gap=1)
    
    # Prediction matrix setup
    #Age = np.linspace(0, 98, 99)[:, None]
    ...
    for offset in range(1, max_offset + 1):
        X_pred = create_regression_matrix(gap=offset)  # shape (n - gap, features)
        pred = reg_model.predict(X_pred)            # shape (n - gap,)
        transformed = recursive_r_transform(pred)   # shape (n - gap,)
    
        diagonals.append(transformed)
        offsets.append(-offset)  # lower diagonal
    
    A_sparse_predict = diags(diagonals, offsets, shape=(n, n), format="csr")
    
    #step_results = recursive_multiply(pred_r)
    #print("Steps:", step_results)
    
    pred_r = A_sparse_predict.diagonal(-1).tolist()
    ...
    "pred_r": pred_r,
    "coef" : coef,
    "A_sparse_predict": A_sparse_predict
    }

    A_dense = A_sparse_predict.toarray()   # careful: only for not-too-big matrices

    plt.figure(figsize=(6,5))
    plt.imshow(A_dense)
    plt.colorbar(label="Value")
    plt.title("Banded matrix values")
    plt.xlabel("Columns")
    plt.ylabel("Rows")
    plt.tight_layout()
    plt.show()
...
# Save all at once
    with open(os.path.join(out_dir, "Velocity",measure, "velocity_objects.pkl"), "wb") as f:
        pickle.dump(velocity_objects, f)
 
    #velocity_list_age, velocity = generate_thrive_lines(
    #    pred_r, start_year=5, end_year=98, velocity_length=2, start_z=-3, end_z=4, space_between_anchors=1, z_thrive=-1.96
    #)
    
    # velocity_list_age, velocity = generate_thrive_lines_conservative(
    #     pred_r, start_year=5, end_year=98, velocity_length=2, start_z=-3, end_z=4, space_between_anchors=1, z_thrive=1.96
    # )
    ...
    #     Z_train = pd.read_pickle(f'/project_cephfs/3022017.06/projects/lifespan_hbr/johbay/Velocity/DATA_NM/trbefile_retrain_{atlas}_adults_ADNI.pkl')
    #     X_train = pickle.load(open(f"/project_cephfs/3022017.06/projects/lifespan_hbr/johbay/Velocity/DATA_NM/X_train_retrain_{atlas}_adults_ADNI.pkl",'rb')).to_numpy()
    #     Y_train = pickle.load(open(f"/project_cephfs/3022017.06/projects/lifespan_hbr/johbay/Velocity/DATA_NM/Y_train_retrain_{atlas}_adults_ADNI.pkl",'rb'))
   
    # else:
    #     Z_train = pd.read_pickle(f'/project_cephfs/3022017.06/projects/lifespan_hbr/johbay/Velocity/DATA_NM/trbefile_{atlas}_demented_adults_ADNI.pkl')
    #     X_train = pickle.load(open(f"/project_cephfs/3022017.06/projects/lifespan_hbr/johbay/Velocity/DATA_NM/X_train_{atlas}_demented_adults_ADNI.pkl",'rb')).to_numpy()
    #     Y_train = pickle.load(open(f"/project_cephfs/3022017.06/projects/lifespan_hbr/johbay/Velocity/DATA_NM/Y_train_{atlas}_demented_adults_ADNI.pkl",'rb'))
    ...
    # model_path = os.path.join(batch_dir, "Models/NM_0_0_estimate.pkl")
    ...
    # synthetic_X0 = np.linspace(min0, max0, 196)[:,None]
    ...
    # #     plt.xlim(min0,max0)
    # synthetic_X = np.concatenate((synthetic_X0, np.full(synthetic_X0.shape,selected_sex_id)),axis = 1)
    ...
    # x_for_thrivelines = np.stack((x_for_thrivelines, np.full(x_for_thrivelines.shape, selected_sex_id)),axis = 1)
    ...
    # # z_for_thrivelines_demented = np.array(OAS2_0157[measure])[:,None]
    ...
    # plt.savefig(f"{velocity_dir}/velocity_plot_{measure}_{model_type}_ADNI_Oct25_up.png",bbox_inches='tight',dpi=300)
    # plt.show()
    # # del model 
    # # del model_be
    # # del model_be_long
...
def make_sparse_correlation_matrix(
    data: pd.DataFrame,
    measure: str,
    ...
    """
    Creates sparse banded correlation and Fisher-transformed matrices for age-based data.
    ...
    - max_band (int): Maximum diagonal band width to compute and store.

    Returns:
    - A_sparse (scipy.sparse.csr_matrix): Sparse banded correlation matrix.
    - Phi_sparse (scipy.sparse.csr_matrix): Sparse banded Fisher-transformed matrix.
    """
    ...
    num_ages = 99
    #if not re_estimate:
        #return diags([], [], shape=(num_ages, num_ages)), diags([], [], shape=(num_ages, num_ages))

    A_diagonals = []
    Phi_diagonals = []
    offsets = []

    # for k in range(-max_band, max_band + 1):
    #     diag_values = []
    #     phi_values = []
    #     for i in range(num_ages - abs(k)):
    #         if k >= 0:
    #             age_i = i
    #             age_j = i + k
    #         else:
    ...
    for k in range(1, max_band+1):
        ...
        for i in range(num_ages - abs(k)):
            ...
            vec1 = data.loc[data["age"] == age_i,["age",measure, "ID_subject", "ID_visit"]]
            vec2 = data.loc[data["age"] == age_j, ["age", measure, "ID_subject", "ID_visit"]]
            
            vec1["ID_subject"] = vec1["ID_subject"].astype(str)
            vec2["ID_subject"] = vec2["ID_subject"].astype(str)
            ...
            if len(overlap_combined) < 8:
                cor = np.nan
            else:
                cor = overlap_combined[f"{measure}_x"].corr(overlap_combined[f"{measure}_y"])

            diag_values.append(cor)
            phi_values.append(fisher_transform(cor))

        A_diagonals.append(np.array(diag_values))
        Phi_diagonals.append(np.array(phi_values))
        offsets.append(-k)

    A_sparse = diags(A_diagonals, offsets, shape=(num_ages, num_ages), format="csr")
    Phi_sparse = diags(Phi_diagonals, offsets, shape=(num_ages, num_ages), format="csr")

    # Visualize correlation matrix (dense conversion for plotting only)
    return A_sparse, Phi_sparse
...
# def get_thrive_lines_conservative(lst, step_results=None, current_result=1, z_thrive = (-1.96) ):
#     if step_results is None:
#         step_results = []
...
#     # Recursively process the rest of the list
#     return get_thrive_lines_conservative(lst[1:], step_results, current_result, z_thrive=z_thrive)
...
def get_subdiagonal(Phi: csr_matrix, gap: int) -> np.ndarray:
    """
    Extract the subdiagonal at a given gap from a sparse matrix.
    Assumes Phi is a square sparse banded matrix.
    """
    return Phi.diagonal(-gap)

# def get_subdiagonal_old(A, gap=1):
#     """   
...
def create_regression_matrix(gap: int, Phi: csr_matrix = None) -> pd.DataFrame:
    """
    ...
    ----------
    gap : int
        The subdiagonal gap (1 = first subdiagonal).
    Phi : scipy.sparse matrix, optional
        If provided, must be a sparse banded matrix from which a diagonal is extracted.

    Returns
    -------
    pd.DataFrame
        Regression design matrix including optionally the target variable `y`.
    """
    # Define age range based on gap
    Age = np.linspace(0, 98 - gap, 99 - gap)[:, None]
    reg_matrix = pd.DataFrame(index=range(Age.shape[0]))

    # If Phi is provided, extract target values from the sparse matrix
    if Phi is not None:
        diagonal = get_subdiagonal(Phi, gap=gap)
        reg_matrix["y"] = diagonal
...
def generate_thrive_lines(
    ...
    Parameters:
    - A (sparse  corelation matrix): List of predicted correlations
    - start_year (int): Starting year.
    - end_year (int): Ending year.
    - velocity_length (int): Length of years thirve lines should predict in the future
    ...
    for i in range(start_year, end_year, space_between_anchors):
        pred_r_snippet = A[i + (velocity_length - 1),i]
        age_range = np.arange(i, i + velocity_length, velocity_length)

        for j in range(start_z, end_z):
            if z_thrive is not None:
                step_results_thrive = list(get_thrive_lines(pred_r_snippet, Z1=j, z_thrive=z_thrive))
            else:
...
def get_thrive_lines(r, Z1, z_thrive):
    #Z_list 
    Z2 = Z1 * r + math.sqrt(1 - r**2) * z_thrive
    return Z1, Z2
...
def generate_thrive_lines_conservative(
    pred_r: list,
    start_year: int,
    end_year: int,
    velocity_length: int,
    ...
    for i in range(start_year, end_year, space_between_anchors):
        ...
        for j in range(start_z, end_z):
            if z_thrive is not None:
                step_results_thrive = list(get_thrive_lines_conservative(pred_r_snippet, current_result=j, z_thrive=z_thrive))
            else:
                step_results_thrive = list(get_thrive_lines_conservative(pred_r_snippet, current_result=j))
